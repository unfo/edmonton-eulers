...
triangles circumference = [(a,b,c) | c <- [1..circumference], b <- [1..c], a <- [1..b], a+b+c == circumference, a^2 + b^2 == c^2]

uniqueTriangle n = 
  let tri = triangles n
  in (length tri) == 1

uniqueTriangles upto = [x | x <- [1..upto], uniqueTriangle x]
main = putStr (show $ length (uniqueTriangles 1500000))
